...
function Simulator(UI, loadModules, generatorConfig, generatorData) {
	...
	var SimulatorWorker = function() {
		...
		if(typeof require === 'function') {
			for(i = 0; i < loadModules.length; i++)
				this.addModule(loadModules[i],'node');
	...
	SimulatorWorker.prototype.start = function(strainId) {
	...
	SimulatorWorker.prototype.end = function(state) {
	...
	SimulatorWorker.prototype.addModule = function(id,moduleArray) {
		var i,n,newModule;
	...
	SimulatorWorker.prototype.addActive = function(id) {
	...
	SimulatorWorker.prototype.removeActive = function(id) {
	...
	SimulatorWorker.prototype.pause = function() {
		this.paused = true;
	};
	SimulatorWorker.prototype.unPause = function() {
		this.paused = false;
		this.tick();
	};

	SimulatorWorker.prototype.addUpgrades = function(module) {
		var levels = [],
			i = 0,
			n = arguments.length;
	...
	SimulatorWorker.prototype.availableUpgrades = function(selectedUpgrades) {
	...
	SimulatorWorker.prototype.purchaseUpgrades = function(upgrades) {
		var upgrade;
	...
	SimulatorWorker.prototype.purchaseMutation = function(mutations) {
	...
	SimulatorWorker.prototype.tick_begin = function(options) {
		var simplifyCof = 1;
	...
	SimulatorWorker.prototype.tick_hordes = function(process, moduleName, options) {
		var simplifyCof = 1;
		for(var i = 0, n = this.hordes.length; i < n; i += simplifyCof) {
			// Don't process every horde every turn. Smaller hordes can be skipped the majority of turns 
			if(i > 0 && i % options.simplifyAt < simplifyCof) {
	...
	SimulatorWorker.prototype.tick_activePoints = function(process) {
	...
	SimulatorWorker.prototype.tick = function() {
	...
	SimulatorWorker.prototype.rendererDecal = function(id, lat, lng, size, texture) {
		UI.rendererDecal(id, lat, lng, size, texture);
	};
	SimulatorWorker.prototype.getPointProperties = function(lat, lng) {
	...
	for(var i = 0, n = generatorData.points.length; i < n; i++)
		if(generatorData.points[i].total_pop > 0)
			SimulatorWorker.prototype.populatedPoints.push(generatorData.points[i]);

    S = new SimulatorWorker(UI, generatorConfig, generatorData);
